...
def _mask_to_polygons(mask, min_area=MIN_AREA):
    """Mask → Close(5×5) → Open(5×5) → Dilate(3×3) → Canny融合 → convexHull → minAreaRect"""
...
def detect_fused(frame_bgr):
    ...
    # HSV 粗定位
    mask_hsv = cv2.inRange(hsv, YELLOW_LOW, YELLOW_HIGH)
    ...
    # 用 HSV 核心区膨胀作为 Canny 的搜索范围（抑制背景噪声）
    k7 = cv2.getStructuringElement(cv2.MORPH_ELLIPSE, (7, 7))
    core_dilated = cv2.morphologyEx(mask_hsv, cv2.MORPH_DILATE, k7, iterations=2)
    ...
    # 融合：HSV 或 边缘
    mask = cv2.bitwise_or(mask_hsv, edges_closed)
